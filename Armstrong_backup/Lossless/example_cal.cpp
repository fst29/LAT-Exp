...
int main() {
	...
	double frequency = 1; // frequency of oscilation
	double A = 0.6*257/4.69; // Amplitude in sensor measurement
	double B = 150; // Amplitude in sensor measurement
	...
	slot_config.kP = 0.0035;
	slot_config.kD = 0;
	slot_config.kI = 0;
	slot_config.kF = 0.0035;
	...
	allConfigs.motionCruiseVelocity = 1000;
	allConfigs.motionAcceleration = 1000;
	...
	// Motion magic settings
	allConfigs0.motionCruiseVelocity = 150;
	allConfigs0.motionAcceleration = 150;
	...
	while (true) {
		...
		if (write_time > 0.01){
			...
			if (loop_time < 1){
				...
				target_position_p = B*(loop_time-2);
				target_position = A*sin(loop_time*PI*2*frequency);
			}
			else if(loop_time >=3 && loop_time<5){
				target_position_p = B-B*(loop_time-3);;
				target_position = A*sin(loop_time*PI*2*frequency);
			}
			else if(loop_time >=5 && loop_time<6){
				target_position_p = -B+B*(loop_time-5);
				target_position = A*sin(loop_time*PI*2*frequency);
			}
			else {
				target_position_p = 0;
