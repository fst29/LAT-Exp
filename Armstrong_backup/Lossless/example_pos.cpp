...
int main() {
	...
	allConfigs.motionCruiseVelocity = 550;
	allConfigs.motionAcceleration = 550;
	allConfigs.motionCurveStrength = 4;
	...
	allConfigs0.velocityMeasurementWindow = 1;
	...
	myfile << "Time," << "Target Position," << "Measured Position," << "Torque," << "Current," <<"Rotation raw," << "Pinion Target," << "Pinion Position," << "Pinion Current," << "Joint Velocity\n";
	...
	while (true) {
		...
		if (write_time > 0.01){
			...
			if (loop_time < 2){
				target_position_p = 0;
				target_position = A*(loop_time);
			}
			else if(loop_time >= 2 && loop_time < 4){
				target_position_p = 0;
				target_position = 100-A*(loop_time-2);
			}
			else if(loop_time >= 4 && loop_time < 6){
				target_position_p = 0;
				target_position = 100-A*(loop_time-4);
			}
			else if(loop_time >=6 && loop_time<8){
				target_position_p = 0;
				target_position = -100+A*(loop_time-6);
			}
			else if(loop_time >=8 && loop_time<10){
				target_position_p = 0;
				target_position = 0;
			}
			else if (loop_time >=10 && loop_time<11){
				target_position_p = 0;
				target_position = A*(loop_time-10);
			}
			else if (loop_time >=10 && loop_time<11){
				target_position_p = 0;
				target_position = 50;
			}
			else if(loop_time >=12 && loop_time<16){
				target_position_p = 0;
				target_position = 100-A*(loop_time-12);
			}
			else if(loop_time >=16 && loop_time<18){
				target_position_p = 0;
				target_position = -100+A*(loop_time-16);
			}
			else if(loop_time >=18 && loop_time<20){
				target_position_p = 0;
				target_position = 0;
			}
			...
			myfile << run_time << "," << target_position << "," << joint_position << "," << torque << "," << current << "," << counter << "," << target_position_p << "," << joint_position_p << "," << current_p << "," << joint_velocity << "\n";
			write_time = 0;
			start = std::chrono::system_clock::now();
		}
		end = std::chrono::system_clock::now();
