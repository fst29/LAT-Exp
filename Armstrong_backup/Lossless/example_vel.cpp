...
int main() {
	...
	double frequency = 0.75; // frequency of oscilation
	double A = 150; // Amplitude in sensor measurement
	double B = 200; // Amplitude in sensor measurement
	...
	while (true) {
		...
		if (write_time > 0.01){
			...
			else if(loop_time >=tCal && loop_time<tCal + 2){
				target_position_p = B*(loop_time-tCal);
				target_position = A*sin((loop_time)*PI*2*frequency);
			}
			else if(loop_time >=tCal + 2 && loop_time<tCal + 4){
				target_position_p = 2*B;
				target_position = A*sin((loop_time)*PI*2*frequency);
			}
			else if(loop_time >=tCal + 4 && loop_time<tCal + 8){
				target_position_p = 2*B-B*(loop_time-tCal - 4);;
				target_position = A*sin((loop_time)*PI*2*frequency);
			}
			else if(loop_time >=tCal + 8 && loop_time<tCal + 10){
				target_position_p = -2*B;
				target_position = A*sin((loop_time)*PI*2*frequency);
			}
			else if(loop_time >=tCal + 10 && loop_time<tCal + 12){
				target_position_p = -2*B+B*(loop_time-tCal - 10);
				target_position = A*sin((loop_time)*PI*2*frequency);
			}
			else if(loop_time >=tCal + 12 && loop_time<tCal + 14){
				target_position_p = 0;
				target_position = A*sin((loop_time)*PI*2*frequency);
			}
			else {
