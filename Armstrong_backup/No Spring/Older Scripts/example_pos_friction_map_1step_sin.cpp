...
int main() {
	...
	allConfigs.motionCruiseVelocity = 200;
	allConfigs.motionAcceleration = 200;
	...
	myfile << "Time," << "Target Position," << "Measured Position," << "Torque," << "Current," <<"Rotation raw," << "Pinion Target," << "Pinion Position," << "Pinion Current," << "Torque demand," << "Previous Pos," << "Moved," << "Motion Direction\n";
	...
	double carriage_pos_vec[4] = {0, 100, 0, 100};
	for (int j = 0; j < 4; j++) {
		carriage_pos = carriage_pos_vec [j];
				...
				if (write_time > 0.01){
					ctre::phoenix::unmanaged::FeedEnable(20);
						if (adjust_time < 1.5 && initial_time > 3){
							...
							if ((j ==1 || j == 3) && initial_time < 2){
								target_position_p = carriage_pos/2*initial_time;
								Joint0.Set(ControlMode::MotionMagic, (target_position_p));
								Joint1.SelectProfileSlot(0, 0);
								target_position = 100*sin(initial_time*PI*2*0.8);
								Joint1.Set(ControlMode::MotionMagic, (target_position));
								start_motion = std::chrono::system_clock::now();
							}
							...
							else if ((j==2) && initial_time < 2){
								target_position_p = 100-100/2*(initial_time);
								Joint0.Set(ControlMode::MotionMagic, (target_position_p));
								Joint1.SelectProfileSlot(0, 0);
								target_position = 100*sin(initial_time*PI*2*0.8);
								Joint1.Set(ControlMode::MotionMagic, (target_position));
								...
								target_position_p = carriage_pos;
								Joint0.Set(ControlMode::MotionMagic, (target_position_p));
								Joint1.SelectProfileSlot(0, 0);
								target_position = 0;
								Joint1.Set(ControlMode::MotionMagic, (target_position));
							...
							if (abs(previousPos-joint_position)>2){
								stop = false;
								moved = 1;
							}
							else {
					...
					myfile << run_time << "," << target_position << "," << joint_position << "," << torque << "," << current << "," << counter << "," << target_position_p << "," << joint_position_p << "," << current_p << "," << torque_demand << "," << staticPos << "," <<moved << "," << motion_direction << "\n";
				}
				write_time = 0;
				start = std::chrono::system_clock::now();
			}
