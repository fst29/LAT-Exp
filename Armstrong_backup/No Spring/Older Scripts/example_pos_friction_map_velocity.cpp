...
int main() {
	...
	myfile << "Time," << "Target Position," << "Measured Position," << "Torque," << "Current," <<"Rotation raw," << "Pinion Target," << "Pinion Position," << "Pinion Current," << "Torque demand," << "Previous Pos," << "Moved," << "Motion Direction," << "i," << "j\n";
		...
		for (int i = 0; i < 10; i++) {
			...
			while (stop) {
				...
				if (write_time > 0.01){
					...
					myfile << run_time << "," << target_position << "," << joint_position << "," << torque << "," << current << "," << counter << "," << target_position_p << "," << joint_position_p << "," << current_p << "," << torque_demand << "," << staticPos <<  "," << motion_direction << "," << i << "," << j << "," << moved << "\n";
					write_time = 0;
					start = std::chrono::system_clock::now();
				}
				end = std::chrono::system_clock::now();
