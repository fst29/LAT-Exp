...
TalonFX Joint1(0); // My current TalonFX motor which I managed to configure with Phoenix Tuner
TalonFX Joint0(1);
...
int main() {
	...
	*/
	...
	double carriage_pos_vec[carriageSteps] = {0};
	double nSteps[carriageSteps] = {coneSteps};
					...
					if (adjust_time < 1.5 && initial_time > 3){
						...
						if ((j!=0) && initial_time < 2){
							...
							if (abs(staticPos-joint_position)>tol){
								...
								torque_demand = torque_demand + 0.0002/4.69*257;
