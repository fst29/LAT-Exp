...
int main() {
	...
	while (true) {
		...
		if (write_time > 0.01){
			...
			else if(loop_time >=2 && <6){
				target_position_p = 0;
				target_position = 100-A*(loop_time-2);
			}
			else if(loop_time >=6 && <8){
				target_position_p = 0;
				target_position = -100+A*(loop_time-6);
			}
			else if(loop_time >=8 && <10){
				target_position_p = 0;
				target_position = 0;
			}
			else if (loop_time >=10 && <12){
				target_position_p = 0;
				target_position = A*(loop_time);
			}
			else if(loop_time >=12 && <16){
				target_position_p = 0;
				target_position = 100-A*(loop_time-12);
			}
			else if(loop_time >=16 && <18){
				target_position_p = 0;
				target_position = -100+A*(loop_time-16);
			}
			else if(loop_time >=18 && <20){
				target_position_p = 0;
				target_position = 0;
			}
