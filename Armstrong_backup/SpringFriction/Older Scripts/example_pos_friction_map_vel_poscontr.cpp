...
int main() {
	...
	allConfigs.motionCruiseVelocity = 150;
	allConfigs.motionAcceleration = 150;
	allConfigs.motionCurveStrength = 10;
		...
		for (int i = 0; i < 10; i++) {
			...
			while (stop) {
				...
				if (write_time > 0.01){
					...
					if (adjust_time < 1.5){
						Joint1.Set(ControlMode::Current, (0));
						previousPos = Joint1.GetSelectedSensorPosition(0);
						moved = 0;
						...
						if (joint_position>750 && motion_direction == 1){
							stop = true;
							motion_direction = -1;
							torque_demand = motion_direction*target_position*(adjust_time-1.5);
							Joint1.Set(ControlMode::MotionMagic, (torque_demand));
						}
					...
					myfile << run_time << "," << target_position << "," << joint_position << "," << torque << "," << current << "," << counter << "," << target_position_p << "," << joint_position_p << "," << current_p << "," << torque_demand << "," << staticPos <<  "," << motion_direction << "\n";
					}
					write_time = 0;
					start = std::chrono::system_clock::now();
				}
