...
int main() {
	...
	double carriage_pos_vec[2] = {0, 150};
	double nSteps[2] = {7, 7};
	for (int j = 0; j < 6; j++) {
				...
				if (write_time > 0.01){
					...
					if (adjust_time < 1.5 && initial_time > 3){
						...
						if ((j!=0) && initial_time < 2){
							target_position_p = 150/2*initial_time + carriage_pos_vec[j-1];;
							Joint0.Set(ControlMode::MotionMagic, (target_position_p));
							Joint1.SelectProfileSlot(0, 0);
						...
						posControl = false;
						if (abs(previousPos-joint_position)>1){
							stop = false;
							moved = 1;
						}
						else {
	...
	// Positive torques negative cone positions
	for (int i = nSteps[j]; i >= -nSteps[j]; i--) {
		//fprintf(stdout, "Carriage id = %u, Rotation id = %u, previous torque demand = %f\r", j, i, torque_demand);
		cout << "Carriage id: " << j << ", Rotation id" << i << ", Previous Torque demand: " << torque_demand << "\r";
		adjust_time = 0;
	...
	// if (i == 0){
	// 		 fprintf(stdout, "TIGHTEN THE SPRING!\r");
	// 		 sleepApp(10000);
	//
	// 	}
	}
